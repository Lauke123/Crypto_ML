...
class Encoder(nn.Module):
    ...
    def __init__(self, input_size: int, embedding_dim:int = 512, output_size:int = 26) -> None:
        ...
        self.output_pins2 = nn.Linear(153, 131)
        self.output_lugs2 = nn.Linear(153, 22)
    ...
    def forward(self, x, inputsize):
        ...
        #out = self.relu(out) (this relu seems to reduce the accuracy of the predicted pins)
        lugs = self.relu(lugs)

        output = torch.cat((out,lugs),1)
        output = torch.transpose(output, 1, 2)
        out = self.output_pins(output)
        output = self.relu(output) # this relu increses the accuracy of predicted lugs
        ...
        output = torch.cat((out,lugs),1)
        output = torch.transpose(output, 1, 2)
        out = self.output_pins2(output)
        output = self.relu(output) # this relu increses the accuracy of predicted lugs
        lugs = self.output_lugs2(output)
        out = torch.transpose(out, 1, 2)
        lugs = torch.transpose(lugs, 1, 2)
